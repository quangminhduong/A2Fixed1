...
public class ShiftDao {
  ...
  private static final String SELECT_BY_USER_ID_FROM_NOW =
    "SELECT * FROM shifts WHERE startTime >= CURRENT_TIMESTAMP AND user_id = ?";
      private static final String SELECT_BY_USER_ID_BEFORE_NOW =
    "SELECT * FROM shifts WHERE startTime <= CURRENT_TIMESTAMP AND user_id = ?";
  ...
  public List<Shift> getFromNowByUserId(long user_id) throws SQLException {
    ...
    public List<Shift> getBeforeNowByUserId(long user_id) throws SQLException {
    Connection connection = DBUtils.getConnection();
    PreparedStatement stm = connection.prepareStatement(SELECT_BY_USER_ID_BEFORE_NOW);
    stm.setLong(1, user_id);
    ResultSet rs = stm.executeQuery();
    List<Shift> shifts = new ArrayList<Shift>();
    while (rs.next()) {
